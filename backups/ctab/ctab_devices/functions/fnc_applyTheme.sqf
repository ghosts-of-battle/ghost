 ...
 *
 * Example:
 * call ghost_ctab_devices_fnc_applyTheme
 */
...
// shaped picture too - it follows the screen's bottom corners
if (!isNull _panel) then {
    _panel ctrlSetTextColor [_r * 0.7, _g * 0.7, _b * 0.7, 0.96];
};

// The page background is a SHAPED PICTURE - it follows the screen's rounded
// bottom corners - so it takes a text colour, not a background colour.
private _page = _display displayCtrl IDC_S7_INFOPAGE;
if (!isNull _page) then {
    _page ctrlSetTextColor [_r * 0.85, _g * 0.85, _b * 0.85, 1];
};
