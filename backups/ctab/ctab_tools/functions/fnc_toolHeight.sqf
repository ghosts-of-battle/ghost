 ...
 * Height map tool.
 *
 * Two points draw a terrain profile between them.
 *
...
localNamespace setVariable ["ghost_ctab_tools_height_points",[]] ;
localNamespace setVariable ["ghost_ctab_tools_height_lastClick",-1] ;
...
private _EH = _map ctrlAddEventHandler ["Draw",{
    ...
    private _lastClick = localNamespace getVariable ["ghost_ctab_tools_height_lastClick",-1] ;
    private _points = localNamespace getVariable ["ghost_ctab_tools_height_points",[]] ;

    if (inputMouse 0 == 1 and (_lastClick + 0.3 < time) and count _points < 2) then {
        localNamespace setVariable ["ghost_ctab_tools_height_lastClick",time] ;
    } ;

    if (inputMouse 0 == 0 and (_lastClick + 0.3 > time)) then {
        _points pushBack (MAPPOS(getMousePosition#0,getMousePosition#1)) ;
        localNamespace setVariable ["ghost_ctab_tools_height_points",_points] ;
        localNamespace setVariable ["ghost_ctab_tools_height_lastClick",-1] ;
        ...
        if (count _points == 2) then {
            ...
            private _cGrp = _display ctrlCreate ["ghost_ctab_tools_heightGraph",73453] ;
            private _return = [] ;
            ...
            localNamespace setVariable ["ghost_ctab_tools_Height_MinMax",[selectMin _return,selectMax _return]] ;
    ...
    if (count _points != 0) then {
        ...
        for "_i" from 0 to 10 do {
            private _length = [100,250] select (_i mod 5 == 0) ;
            private _posLocal = [
        ...
        if (count _points == 2) then {
            ...
            if (abs (_height / _mapScale) < 250 and (_curDist/_length > 0 and _curDist/_length < 1)) then {
                ...
                (localNamespace getVariable ["ghost_ctab_tools_Height_MinMax",[-1,-1]]) params ["_min","_max"] ;
