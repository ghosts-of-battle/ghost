 ...
 * Grid map tool.
 *
 * Lays a numbered reference grid over an area.
 *
...
localNamespace setVariable ["ghost_ctab_tools_placeGrid_lastClick",-1];
localNamespace setVariable ["ghost_ctab_tools_placeGrid_dragging",-1];
...
private _EH = _map ctrlAddEventHandler ["Draw",{
    ...
    private _lastClick = localNamespace getVariable ["ghost_ctab_tools_placeGrid_lastClick",-1];
    private _points = missionNamespace getVariable ["ghost_ctab_tools_placeGrid_points",[]];
    ...
    if (inputMouse 0 >= 1) then {
        ...
        localNamespace setVariable ["ghost_ctab_tools_placeGrid_lastClick",time];
    };
    ...
    if (inputMouse 0 == 0 && (_lastClick + 0.1 > time)) then {
        ...
        private _marker = createMarker [format ["ghost_ctab_tools_Grid_%1_L",_random],
        _pointNE vectorAdd [2,-_height/2],currentChannel,player];
        _marker setMarkerSize [2,_height/2-4];
        _markers pushBack _marker;

        private _marker = createMarker [format ["ghost_ctab_tools_Grid_%1_R",_random],
        _pointNE vectorAdd [-_width-2,-_height/2],currentChannel,player];
        _marker setMarkerSize [2,_height/2-4];
        _markers pushBack _marker;

        private _marker = createMarker [format ["ghost_ctab_tools_Grid_%1_T",_random],
        _pointNE vectorAdd [-_width/2,-2],currentChannel,player];
        _marker setMarkerSize [-_width/2,2];
        _markers pushBack _marker;
        
        private _marker = createMarker [format ["ghost_ctab_tools_Grid_%1_B",_random],
        _pointNE vectorAdd [-_width/2,-_height+2],currentChannel,player];
        _marker setMarkerSize [-_width/2,2];
        _markers pushBack _marker;
        ...
        for "_i" from 0 to (_scale#0)-1 do {
            if (_i != 0) then {
                private _marker = createMarker [
                    format ["ghost_ctab_tools_Grid_%1_X%2",_random,_i],
            ...
            private _marker = createMarker [
                format ["ghost_ctab_tools_Grid_%1_X%2_text",_random,_i],
        ...
        for "_i" from 0 to (_scale#1)-1 do {
            if (_i != 0) then {
                private _marker = createMarker [
                    format ["ghost_ctab_tools_Grid_%1_Y%2",_random,_i],
            ...
            private _marker = createMarker [format ["ghost_ctab_tools_Grid_%1_Y%2_text",_random,_i],
        ...
        for "_xx" from 0 to (_scale#0)-1 do {
            for "_yy" from 0 to (_scale#1)-1 do {
                private _marker = createMarker [
                    format ["ghost_ctab_tools_Grid_%1_gridTitle_%2_%3",_random,_xx,_yy],
                    _pointNE vectorAdd [
        ...
        // GHOST: BCE read the colour off a list box beside its own map. We have no
        // such control - and a handheld has nowhere to put one - so it is a setting.
        private _color = call FUNC(markerColour) ;
        ...
        //- BoardCast : ["ownerID:netID", Marker Count]
            private _PLP_GLB_Var = missionNamespace getVariable ["ghost_ctab_tools_Markers",createHashMap];
            ...
            missionNamespace setVariable ["ghost_ctab_tools_Markers",_PLP_GLB_Var,true];

            if (_iscTab) then {
                if (!isNil "cTab_fnc_updateUserMarkerList") then {call cTab_fnc_updateUserMarkerList};
            };
    };

    //-Get Color RGBs
    private _color = +((call FUNC(markerColour)) call FUNC(markerRGBA)) ;
    _color set [3,1];
    ...
    missionNamespace setVariable ["ghost_ctab_tools_placeGrid_points",_points];
}];
