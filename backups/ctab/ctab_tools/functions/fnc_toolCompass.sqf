 ...
 * Compass map tool.
 *
 * Drop an origin, then read bearing and range to the cursor. Hold right mouse for mils.
 *
...
localNamespace setVariable ["ghost_ctab_tools_compass_origPos",[-1,-1]] ;
localNamespace setVariable ["ghost_ctab_tools_compass_lastClick",-1] ;
...
private _EH = _map ctrlAddEventHandler ["Draw",{
    ...
    private _lastClick = localNamespace getVariable ["ghost_ctab_tools_compass_lastClick",-1] ;
    private _origPos = localNamespace getVariable ["ghost_ctab_tools_compass_origPos",[-1,-1]] ;

    if (inputMouse 0 == 1 and (_lastClick + 0.3 < time)) then {
        localNamespace setVariable ["ghost_ctab_tools_compass_lastClick",time] ;
    } ;
    if (inputMouse 0 == 0 and (_lastClick + 0.3 > time)) then {
        localNamespace setVariable ["ghost_ctab_tools_compass_origPos",(MAPPOS(getMousePosition#0,getMousePosition#1))] ;
        localNamespace setVariable ["ghost_ctab_tools_compass_lastClick",-1] ;
    } ;
