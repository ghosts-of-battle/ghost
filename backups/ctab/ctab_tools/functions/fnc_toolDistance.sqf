 ...
 * Distance map tool.
 *
 * Click to lay a chain of points; the legs and the running total are drawn as you go.
 *
...
localNamespace setVariable ["ghost_ctab_tools_distance_distancePoints",[]] ;
localNamespace setVariable ["ghost_ctab_tools_distance_lastClick",-1] ;
localNamespace setVariable ["ghost_ctab_tools_distance_dragging",-1] ;
...
private _EH = _map ctrlAddEventHandler ["Draw",{
    ...
    private _lastClick = localNamespace getVariable ["ghost_ctab_tools_distance_lastClick",-1] ;
    private _distancePoints = localNamespace getVariable ["ghost_ctab_tools_distance_distancePoints",[]] ;
    private _dragging = localNamespace getVariable ["ghost_ctab_tools_distance_dragging",-1] ;

    if (inputMouse 0 == 1 and (_lastClick + 0.3 < time)) then {
        localNamespace setVariable ["ghost_ctab_tools_distance_lastClick",time] ;
    } ;
    if (inputMouse 0 == 0 and (_lastClick + 0.3 > time)) then {
        _distancePoints pushBack (MAPPOS(getMousePosition#0,getMousePosition#1)) ;
        localNamespace setVariable ["ghost_ctab_tools_distance_distancePoints",_distancePoints] ;
        localNamespace setVariable ["ghost_ctab_tools_distance_lastClick",-1] ;
    } ;
    ...
    private _mPos = (MAPPOS(getMousePosition#0,getMousePosition#1)) ;
    {
        if (_x distance2D _mPos < 35*_mapScale and inputMouse 0 <= 1) then {
            localNamespace setVariable ["ghost_ctab_tools_distance_dragging",_forEachIndex] ;
        } ;
