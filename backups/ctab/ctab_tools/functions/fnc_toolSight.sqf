...
/*
 * Line of Sight map tool.
 *
 * Traces what an observer at the base point can and cannot see.
 *
...
localNamespace setVariable ["ghost_ctab_tools_LineOfSight_basePos",[-1,-1]] ;
localNamespace setVariable ["ghost_ctab_tools_LineOfSight_lastClick",-1] ;
localNamespace setVariable ["ghost_ctab_tools_LineOfSight_dirDist",[-1,-1]] ;
...
private _EH = _map ctrlAddEventHandler ["Draw",{
    ...
    // GHOST: every other tool has this and this one did not - without it the
    // tool reacts to clicks made anywhere on a device screen, including on its
    // own buttons.
    ...
    private _lastClick = localNamespace getVariable ["ghost_ctab_tools_LineOfSight_lastClick",-1] ;
    private _basePos = localNamespace getVariable ["ghost_ctab_tools_LineOfSight_basePos",[-1,-1]] ;
    private _calcData = localNamespace getVariable ["ghost_ctab_tools_LineOfSight_calcData",createHashMap] ;

    if (inputMouse 0 == 1 and (_lastClick + 0.3 < time)) then {
        localNamespace setVariable ["ghost_ctab_tools_LineOfSight_lastClick",time] ;
        localNamespace setVariable ["ghost_ctab_tools_LineOfSight_savedLOS",[]] ;

        // reset evaluated data
        localNamespace setVariable ["ghost_ctab_tools_LineOfSight_calcData",createHashMap] ;
    } ;
    if (inputMouse 0 == 0 and (_lastClick + 0.3 > time)) then {
        _basePos = (MAPPOS(getMousePosition#0,getMousePosition#1)) ;
        localNamespace setVariable ["ghost_ctab_tools_LineOfSight_basePos",_basePos] ;
        localNamespace setVariable ["ghost_ctab_tools_LineOfSight_lastClick",-1] ;
    } ;
