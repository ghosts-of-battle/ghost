...
/*
 * Mark Buildings map tool.
 *
 * Numbers the buildings under the cursor, so a callsign can name one.
 *
...
//missionNamespace setVariable ["ghost_ctab_tools_markHouses_points",[]] ;
localNamespace setVariable ["ghost_ctab_tools_markHouses_lastClick",-1] ;
localNamespace setVariable ["ghost_ctab_tools_markHouses_curPoints",[]] ;
...
private _EH = _map ctrlAddEventHandler ["Draw",{
    ...
    private _lastClick = localNamespace getVariable ["ghost_ctab_tools_markHouses_lastClick",-1] ;
    private _points = missionNamespace getVariable ["ghost_ctab_tools_markHouses_points",[]] ;
    private _curPoints = localNamespace getVariable ["ghost_ctab_tools_markHouses_curPoints",[]] ;
    ...
    if (inputMouse 0 >= 1) then {
        if (_lastClick == -1) then {
            ...
            localNamespace setVariable ["ghost_ctab_tools_markHouses_curPoints",_curPoints] ;
        } ;
        localNamespace setVariable ["ghost_ctab_tools_markHouses_lastClick",time] ;
    } ;
    if (inputMouse 0 == 0 && (_lastClick + 0.1 > time)) then {
        ...
        private _markers = [] ;
        {
            ...
            private _marker = createMarker [
                ["ghost_ctab_tools_markHouses_",_hash] joinString "",
            ...
            private _marker = createMarker [format ["ghost_ctab_tools_markHouses_%1_L",_hash],
        ...
        missionNamespace setVariable ["ghost_ctab_tools_markHouses_points",_points] ;
    ...
    if (count _curPoints != 0 && inputMouse 0 == 2) then {
        ...
        _color = +(_color call FUNC(markerRGBA)) ;
        _color set [3,1] ;
