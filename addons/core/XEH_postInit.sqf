    ...
    ["unit", {
        ...
        if ((getPlayerUID player) in AUTHORIZED_USERS) then {
            if (!GVAR(playerAuth)) then {
                ...
                diag_log text format ["[GHOST-core]: Player UID [%1] in admin list", getPlayerUID player];
            };
        };
        if ((getPlayerUID player) in TECH_USERS) then {
            if (!GVAR(playerTech)) then {
                GVAR(playerTech) = true;
                diag_log text format ["[GHOST-core]: Player UID [%1] in tech list", getPlayerUID player];
            };
        };
...
["ghost_adminMsg", {
    ...
    if (hasInterface) then {
        if ((_to == profileName) || {_to == "#ALL"} || {(GVAR(showNotifcations)) && {[] call FUNC(isAuthorized)}}) then {
            systemChat format ["GHOST [%1>%2]: %3", _from, _to, _msg];
        };
    };
    if (isServer) then {
        diag_log text format ["[GHOST] ghost_adminMsg EH: %1", _this];
    };
...
["ghost_missionHint", {
    ...
    TRACE_3("ghost_missionHint eh",_msg,_side,_rankMin);
    if (hasInterface) then {
    ...
    if (isServer) then {
        diag_log text format ["[GHOST] ghost_missionHint EH: %1", _this];
    };
}] call CBA_fnc_addEventHandler;
