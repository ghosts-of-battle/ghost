...
if (isServer) then {
    ...
    [{
...

        {
            ...
            if (!((missionNamespace getVariable [_settingName, -999]) isEqualTo _settingValue)) then {
                ...
                ["ghost_adminMsg", [_log, "Mission"]] call CBA_fnc_globalEvent;
            };
        } forEach _settings;
    ...
    if (isServer && {!hasInterface}) then {
        [{
            ...
            if (_size > 0) then {
                ["ghost_adminMsg", [format ["Warning: %1 settings set in profileNamespace", _size], "Mission"]] call CBA_fnc_globalEvent;
            };
