 ...
 *
 * Example:
 * [ [450, 60, 3, [target1, target2], [invalidTarget1, invalidTarget2], controller, [controller1, controller2], "range", 1, [trigger1, trigger2]], idPFH] call ghost_shootingrange_fnc_popupPFH;
 *
 * Public: No
 */
...
if (_mode in [1, 2, 5] && {_currentTime >= _timeStart + _duration}) exitWith {
    [_idPFH, _controller, _controllers, _name, _targets, _targetsInvalid, _mode, true, GVAR(currentSmain), GVAR(maxSmain)] call FUNC(popupPFHexit);
};
...
if ((_mode == 3 && {GVAR(targetNumber) >= _targetAmount}) || {_mode > 3 && {GVAR(targetNumber) >= count _targets}}) exitWith {
    ...
    [_idPFH, _controller, _controllers, _name, _targets, _targetsInvalid, _mode, true, GVAR(currentSmain), GVAR(maxSmain), _timeElapsed, _triggers] call FUNC(popupPFHexit);
};
