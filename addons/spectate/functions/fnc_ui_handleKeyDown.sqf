 ...
 *
 * Examples:
 * [nil,54] call ghost_spectate_fnc_ui_handleKeyDown;
 *
 * Public: No
 */
...
if ((_key == DIK_F4) && {_this select 2} && {_this select 3}) exitWith {
    diag_log text format ["[GHOST] - Warning opening old cam sepctator"];
    call FUNC(exit);
...
if ((_key == DIK_F5) && {_this select 2} && {_this select 3}) exitWith {
    diag_log text format ["[GHOST] - Opening Testing Menu"];
