...
private _curHeadlampObject = _unit getVariable ['Headlamps_LampObject', objNull];
if (!(isNull _curHeadlampObject)) then {
    ...
    private _headwearHeadlampPresetName = '';
    if (!Headlamps_Settings_AllowAll) then {
        private _currentHeadwear = headgear _unit;
        ...
        if (_headwearHeadlampPresetName == '') then {
            private _foundIndex = Headlamps_ExtraCompatibleHeadwear findIf { (_x select 0) == _currentHeadwear };
    ...
    private _preset = (configFile >> 'Headlamps_Presets' >> _headwearHeadlampPresetName);
    if (!(isClass _preset)) exitWith {};

    private _colorCycle = getArray (_preset >> 'colorCycle');
    private _currentLampVariant = _unit getVariable ['Headlamps_LampVariant', ''];
    if (_currentLampVariant == '') then {
        _currentLampVariant = _colorCycle select 0;
    };

    private _offset = getArray (_preset >> 'offset');
