...
[QGVAR(resetGear), {
    ...
    diag_log text format ["[GHOST] Resetting Gear on %1 [%2]", (name _unit), _unit];
    ...
    [{
        if (missionNamespace getVariable [QEGVAR(assignGear,useGhost), false]) then {
            diag_log text format ["[GHOST] Calling %1", QEFUNC(assignGear,requestPlayerGear)];
            [_this] call EFUNC(assignGear,requestPlayerGear);
        } else {
            diag_log text format ["[GHOST] Calling F_fnc_assignGearMan"];
...
[QGVAR(resetSpectator), {
    ...
    diag_log text format ["[GHOST] Resetting spectator on %1 [%2]", (name _unit), _unit];
    if (isNull _unit || {!local _unit} || {alive _unit && !(_unit isKindOf QEGVAR(spectate,spectator))}) exitWith {};

    diag_log text format ["[GHOST] Calling %1", QEFUNC(spectate,exit)];
    [] call EFUNC(spectate,exit);

    diag_log text format ["[GHOST] Calling %1", QEFUNC(spectate,setup)];
...
[QGVAR(resetMarkers), {
    ...
    diag_log text format ["[GHOST] Resetting markers on %1 [%2]", (name _unit), _unit];
    if (isNull _unit || {!local _unit}) exitWith {};

    diag_log text format ["[GHOST] Calling %1", QEFUNC(markers,initMarkerHash)];
