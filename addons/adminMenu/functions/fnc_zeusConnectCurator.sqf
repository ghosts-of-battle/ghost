...
if (_giveZeus) then {
        ...
        private _canSkipWaiting = false;
        {
            ...
            if (isNull _assignedUnit) then {
                ...
                if !(isPlayer _assignedUnit) then {
                    ...
                    unassignCurator _x;
                    diag_log text format ["[GHOST] Unassigning non-player [%1](%2) from Zeus [%3]", _assignedUnit, typeOf _assignedUnit, _x];
                };
            };
        } forEach allCurators;
        ...
        [
            ...
            },
            {
                ...
                if (isNull _zeusModule) then { //Only create a new zeus module if no free available
                    diag_log text format ["[GHOST] No free Zeus module found, creating new"];
                ...
                diag_log text format ["[GHOST] Assigning [%1] to Zeus [%2]", name _unit, _zeusModule];
                _unit assignCurator _zeusModule;
                ["ghost_becomeZeus", [_unit], [_unit]] call CBA_fnc_targetEvent;
    ...
    if (!isNull _zeusModule) then {
        diag_log text format ["[GHOST] Unassigning [%1] from Zeus [%2]", name _unit, _zeusModule];
