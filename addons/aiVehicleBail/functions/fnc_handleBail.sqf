 ...
 *
 * Example:
 * [tank1, false, true] call ghost_aiVehicleBail_fnc_determineGenericSelectionType
 *
 *
 * Public: No
 */
...
if (!_canMove && !_canShoot) exitWith { // If you can't move and you can't shoot, you better GTFO
    ...
    diag_log text format["[GHOST] [%1] is a sitting duck and is bailing [%2 | %3]", _vehicle, _canMove, _canShoot];
};

if (!_canShoot) exitWith {
    if (0.5 > _rand) then { // 50% chance of bailing out if turret/gun is destroyed
        [_vehicle] spawn FUNC(abandon);
        diag_log text format["[GHOST] [%1] Cannot shoot and is bailing with chance [%2] [%3 | %4]", _vehicle, _rand, _canMove, _canShoot];
    } else {
        _vehicle allowFleeing 1;
        diag_log text format["[GHOST] [%1] Cannot shoot and is fleeing with chance [%2] [%3 | %4]", _vehicle, _rand, _canMove, _canShoot];
    };
};
...
if (!_canMove) exitWith {
    if (0.8 > _rand) then { // 80% Chance of bailing out if engine is destroyed
        [_vehicle] spawn FUNC(abandon);
        diag_log text format["[GHOST] [%1] Cannot move and is bailing with chance [%2] [%3 | %4]", _vehicle, _rand, _canMove, _canShoot];
    } else {
        diag_log text format["[GHOST] [%1] Cannot move and is bunkering with chance [%2] [%3 | %4]", _vehicle, _rand, _canMove, _canShoot];
    };
};
