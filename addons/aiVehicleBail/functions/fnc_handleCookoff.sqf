 ...
 *
 * Example:
 * [tank2] call ghost_aiVehicleBail_fnc_handleCookoff;
 *
 *
 * Public: No
 */
...
if (!_alreadyDetonating && { _chanceOfDetonate > random 1 }) exitWith {
    ...
    diag_log text format["[GHOST] (cookoff) Detonating [%1] with a chance-to-detonate [%2] hit [%3]", _vehicle, _chanceOfDetonate, _hitPoint];
...
if (!_alreadyCookingOff && { _chanceOfFire > random 1 }) exitWith {
    ...
    if (!_alreadyDetonating && { [_vehicleConfig >> QGVAR(detonationDuringFireProb), "number", 0] call CBA_fnc_getConfigEntry > random 1 }) then {
        [_vehicle, _currentVehicleAmmo] call FUNC(detonate);
        diag_log text format["[GHOST] (cookoff) Detonating [%1] while cooking off with a chance of [%2] hit [%3]", _vehicle, [_vehicleConfig >> QGVAR(detonationDuringFireProb), "number", 0] call CBA_fnc_getConfigEntry, _hitPoint];
        _vehicle setVariable [QGVAR(detonating), true];
    };
    _vehicle setVariable [QGVAR(cookingOff), true];
    diag_log text format["[GHOST] (cookoff) Cooking-off [%1] with a chance-of-fire [%2] hit [%3]", _vehicle, _chanceOfFire, _hitPoint];
    [_vehicle] spawn FUNC(abandon);
    diag_log text format["[GHOST] [%1] is on fire is bailing", _vehicle];
};

// Avoid RPT spam
if (_alreadyDetonating || _alreadyCookingOff) exitWith { };

diag_log text format["[GHOST] (cookoff) [%1] No Cook-off, no detonations (Chance of fire [%2] Chance of detonation [%3] Incendiary [%4] Warhead Type [%5])", _vehicle, _chanceOfFire, _chanceOfDetonate, _incendiary, _warheadType];
