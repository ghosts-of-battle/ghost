...
if (isServer) then {
    ...
    [{
        ...
        if ((isNil "_missionTestingType") || (_missionTestingType == 0)) then {
            ...
            // can override with global exec: ghost_miscMedical_isTVT = false;
        } else {
    ...
    [{
        ...
        case ("injured"): {
                if (_isUncon || {_inputD > -1}) then {
                    ...
                    if (GVAR(readyToMsg)) then {
                        ["ghost_adminMsg", [format ["[AUTO] bugged A"], profileName]] call CBA_fnc_globalEvent;
        ...
        case ("unconscious"): {
                if ((!_isUncon) || {_inputD == -1}) then {
                    ...
                    if (GVAR(readyToMsg)) then {
                        ["ghost_adminMsg", [format ["[AUTO] bugged B"], profileName]] call CBA_fnc_globalEvent;
                        GVAR(readyToMsg) = false;
                    };
                };
            };
