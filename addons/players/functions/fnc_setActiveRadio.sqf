 ...
 *
 * Example:
 * ["ACRE_PRC343"] call ymf_fnc_setActiveRadio
 * ["TFAR_anprc152"] call ymf_fnc_setActiveRadio
 *
 * Public: No
 */

params [["_radio", 'ACRE_PRC148', ['ACRE_PRC148']]];
...
if (EGVAR(patches,usesTFAR)) exitWith {
    [{
        ...
        {
            private _baseRadio = [_x] call EFUNC(systems,getRadioBase);
            if (_x == _baseRadio) then { continue }; // skip base radios (was exitWith - broke the whole loop)
            if (_x == _radio) then { continue };     // skip the excluded radio
            _activeRadio = _x;
            break;
        } forEach _allRadios;
