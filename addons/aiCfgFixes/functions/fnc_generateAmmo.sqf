 ...
 *
 * Example:
 * [] call ghost_aiCfgFixes_fnc_generateAmmo;
 *
 * Public: Yes
 */
...
private _retAmmo = [];
{
    ...
    if (!(configName(_x) isEqualTo "")) then {
        if (configName(inheritsFrom(_x)) isEqualTo "") then {
            ...
            {
                if (count configProperties[_currentAmmo, "configName(_x) isEqualTo 'ghost_aiCfgFixes_macroUsed'", false] <= 0) then {
                    _newData pushBack [configName(_x), str(getNumber(_x))];
                } else {
                    private _macro = getText(configProperties[_currentAmmo, "configName(_x) isEqualTo 'ghost_aiCfgFixes_macroUsed'", false] select 0);
...
private _setAiUsageFlags = {
    ...
    _retAmmo = [_retAmmo, "ghost_aiCfgFixes_he_rocket",    [64, 128, 512],         [["cost", "100"]]] call _setUsageFlags;
