...
#define AMMO_CLASS(type) SCORE_3(ghost,bullet,type)
#define QAMMO_CLASS(type) QUOTE(AMMO_CLASS(type))
#define MAGAZINE_CLASS(type) SCORE_2(ghost,type)
...
#define DISPLAY_NAME(ammoDisplayName,bulletCount) QUOTE(CONCAT(æ ,CONCAT(ammoDisplayName,CONCAT(bulletCount,Rnd (No Tracer) [GHOST]))))
#define TRACER_DISPLAY_NAME(ammoDisplayName,bulletCount,colour) QUOTE(CONCAT(æ ,CONCAT(ammoDisplayName,CONCAT(bulletCount,Rnd Tracer (REPEAT(colour)) [GHOST]))))
#define RELOAD_TRACER_DISPLAY_NAME(ammoDisplayName,bulletCount,colour) QUOTE(CONCAT(æ ,CONCAT(ammoDisplayName,CONCAT(bulletCount,Rnd Reload Tracer (REPEAT(colour)) [GHOST]))))
