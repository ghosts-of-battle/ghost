...

{
	call {
		...
		if (_x select 0 == "mode") exitWith {
			...
			_displayItems = call {				
				if (_displayName in ["cTab_Tablet_dlg", QGVAR(gft_dlg)]) exitWith {
					...
					467540,467541,467542,467543,467544,467545,467546,
					467320,467390,467391,467395,467396,
					467700,467701,467702,467703,467720,467721,467730,
				...
				if (_displayName in ["cTab_Android_dlg", QGVAR(s23_dlg)]) exitWith {
					// GHOST: our pages, menus and alarm buttons join the hide list
					[467100,467101,467102,467200,467201,467300,467310,467401,467410,
					467500,467501,467502,467503,467504,467505,467506,467507,467508,467509,
					467510,467511,467512,467513,467514,467515,467516,467517,
			...
			if (_displayItems isNotEqualTo []) then {
				...
				call {
					...
					if (_mode in ["ghost_ctab_s7_weather","ghost_ctab_s7_drone","ghost_ctab_s7_jam","ghost_ctab_s7_mesh","ghost_ctab_s7_radio","ghost_ctab_s7_alarm","ghost_ctab_s7_settings"]) exitWith {
						_displayItemsToShow = [467200,467201];
						if (_mode == "ghost_ctab_s7_alarm") then {
						...
						if (_mode == "ghost_ctab_s7_settings") then {
							...
							// GHOST: and cTab's map-tool switches, which the
							// handhelds have no bezel key for
							_displayItemsToShow append [467540,467541,467542,467543,467544,467545,467546];
						};
						if (_mode == "ghost_ctab_s7_drone") then {
							_displayItemsToShow append [467720,467721];
						};
						if (_mode == "ghost_ctab_s7_weather") then {
							// GHOST: the current-condition glyph
							_displayItemsToShow pushBack 467730;
						};
						_btnActCtrl ctrlSetTooltip "";
					};
					if (_mode in ["ghost_ctab_s7_uav","ghost_ctab_s7_hcam"]) exitWith {
					...
					if (_mode == "ghost_ctab_s7_hack") exitWith {
						_displayItemsToShow = [
							8702,8703,8704,8705,8706,8707,8708,8709,8710,
							8720,8721,8722,8723,8725,8726,8727,8728,8730,8731,8732,8733,
							8735,8736,8737,8738,8740,8741,8742,8743,8745,8746,8747,8748,
					...
					if (_mode == "DESKTOP") exitWith {
						...
						// GHOST: the tablet's app panel and its time/weather widget
						_displayItemsToShow append [467310,467401,467410];
						// GHOST: the GFT-1 has its own menu, and it opens
						// from its key rather than sitting on the desktop
						_displayItemsToShow pushBack 467390;
						_displayItemsToShow pushBack 467395;
					...
					if (_mode == "BFT") exitWith {
						...
						if (profileNamespace getVariable ["ghost_ctab_s7_scanPanel", true]) then {
			...
			if (_mode == "UAV") then {
				...
				// Populate list of UAVs
				{
					if (crew _x isNotEqualTo []) then {
						_index = _uavListCtrl lbAdd format ["%1:%2 (%3)",groupId group _x,groupId _x,getText (configOf _x >> "displayname")];
