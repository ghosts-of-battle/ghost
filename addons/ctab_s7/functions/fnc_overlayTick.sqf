...
private _jamIcon = _display displayCtrl IDC_S7_JAMICON;
if (!isNull _jamIcon) then {
    _jamIcon ctrlSetTextColor ([_green, _amber, _red] select _jamState);
};

private _meshIcon = _display displayCtrl IDC_S7_MESHICON;
if (!isNull _meshIcon) then {
    _meshIcon ctrlSetTextColor ([_red, _green] select (_mesh > 0));
};
