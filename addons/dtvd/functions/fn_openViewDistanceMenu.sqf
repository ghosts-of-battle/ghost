...
// The labels used to be built into a stringtable key at runtime
// (localize format ["STR_DTVD_terrainGrid%1",_x]). With the strings inlined
// there is no table to look up, so the label sits beside the value it belongs
// to. Unknown values fall back to the number itself rather than throwing.
private _gridLabels = ["50 (Smoothest)","25 (Default)","12.5","6.25","3.125 (Best Quality)"];
private _gridIndex = _gridData find (str DT_terrainGridMax);
_gridData = _gridData select [_gridIndex,5];
_gridLabels = _gridLabels select [_gridIndex,5];

for "_i" from 2100 to 2104 do {
    private _control = _display displayCtrl _i;
    {
        _control lbAdd (_gridLabels param [_forEachIndex,_x]);
