...
class CfgPatches
{
	class 2035_MRP
	{
		units[] = {};
		weapons[] = {};
		requiredVersion = 1;	//以上三个用途不明，保留即可
		requiredAddons[] = {};	//前置模组，没有则留空
	};
};
...
class cfgWeapons
{
	...
	//无盔罩Highcut基础类
	class opscore_highcut_base: H_HelmetB	//左为所制作装备的注册名，右为换皮的基础类，照抄就行
	{
		author = "Kiyo";
		scope = 1;	//定义在编辑器或军火库的显示方式：0-private(不知道什么意思)，1-不可见，2-公开可见
		displayName = "HighCut Helmet Base";	//游戏内显示名称
		picture = "\2035mrp\UI\Helmet\ui_opscore_sample.paa";	//游戏内物品栏icon
		model = "\2035mrp\Model\Helmet\opscore_HighCut_1.p3d";	//模型路径
		hiddenSelections[] = {"Camo1","flag_left","headset","MPLS","battery"};	//贴图的组成模块，可在OB中查询模块名
		...
		class ItemInfo: HeadgearItem
		{
			...
			class HitpointsProtectionInfo
			{
				class Head
				{
					...
					armor = 8;	//原版头盔防护参考值：I级4/0.5、II级6/0.5、III级8/0.5、IV级10/0.5、V级12/0.5
					passThrough = 0.5;
				};
			};
		};
