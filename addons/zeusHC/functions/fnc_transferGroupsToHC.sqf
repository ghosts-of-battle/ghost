 ...
 *
 * Arguments:
 * 0: Force all units onto the headless clients? If false, will only move groups with the ghost_zeusHC_addGroupToHC flag set <BOOL>
 *
 * Examples:
 * [true] call ghost_zeusHC_fnc_transferGroupsToHC; //move them all
 * [] call ghost_zeusHC_fnc_transferGroupsToHC; //only move flagged
 *
 * Public: Yes
 */
...
if (!isMultiplayer) exitWith {
    diag_log text format ["[GHOST] Not in multiplayer, exiting %1",QFUNC(transferGroupsToHC)];
    diag_log text format ["[GHOST] Broadcasting aiTransfered var (SKIPPED)"];
...
if (_allHCs isEqualTo []) exitWith {
    diag_log text format ["[GHOST] No headless clients found, exiting %1", QFUNC(transferGroupsToHC)];
    diag_log text format ["[GHOST] Broadcasting aiTransfered var (SKIPPED)"];
    missionNameSpace setVariable [QGVAR(aiTransfered), true, true];
};
...
diag_log text format ["[GHOST] Broadcasting aiTransfered var (Transfer Done)"];
missionNameSpace setVariable [QGVAR(aiTransfered), true, true];
