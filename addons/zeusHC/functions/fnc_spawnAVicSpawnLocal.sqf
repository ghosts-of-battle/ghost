 ...
 *
 * Examples:
 * [GVAR(vicSpawnLocation), _vehicleClass, _side] call ghost_zeusHC_fnc_spawnAVicSpawnLocal
 * [getPos player, "B_MBT_01_cannon_F", west] call ghost_zeusHC_fnc_spawnAVicSpawnLocal
 *
 * Public: Yes
 */
...
private _crewType = if (isText (configFile >> "CfgVehicles" >> _vehicleToAdd >> "crew")) then {
    ...
    switch (_side) do {
        case (west): { "ghost_w_vicc" };
        case (east): { "ghost_e_crew" };
        case (resistance): { "ghost_i_vicc" };
