/*
 * Author: BadWolf
 * Function is called by the user at the time that they want to use the checklist. This can be called either using the ACE Self interact, or in Zues from the GHOST Modules Section.
 *
 * Arguments:
 *
 * Examples:
 * [] call ghost_missionTesting_fnc_displayMenu;
 *
 * Public: No
 */
...
if (!is3DEN) then {
    if (!EGVAR(spectate,running)) then {
        ...
        _killGoToSpec ctrlSetTooltip "Kill player and go to GHOST spectator if enabled (which it should be enabled else MM messed up)";
...
if (is3DEN) then {call compileScript ['\z\ghost\addons\missiontesting\XEH_postInit.sqf']};
