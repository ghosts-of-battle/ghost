...
// [] call compileScript ["\z\ghost\addons\missionMaking\functions\fnc_testAllLoadouts.sqf"]

params [["_testIndex", -1, [0]]];

private _loadouts = "true" configClasses (missionConfigFile >> "ghost_checkLoadouts");
...

    {
...

        {
            ...
            
            {
                ...
                private _weaponMagsGhost = _weaponMags select {(_x select [0,7]) == "ghost_"};
                ...
                // Check we use ghost mags if they exist for the weapon
                if ((_weaponMagsGhost isNotEqualTo []) && {(_configMagazines arrayIntersect (_weaponMags - _weaponMagsGhost)) isNotEqualTo []}) then {
                    private _currentMags = _configMagazines arrayIntersect _weaponMags;
                    if ([format ["%1 using non-ghost mags", _x], _currentMags] call _fnc_error) then {
                        {
                            ...
                            private _similar = _weaponMagsGhost select {
                            ...
                            if (_similar isEqualTo []) then { 
                                _similar = _weaponMagsGhost select {
                                    (_magCount == getNumber (_cfgMag >> _x >> "count"))
                                    && {(_magColor == "") || {_magColor in toLower _x}}
                                };
                            };
                            if (_similar isEqualTo []) then { _similar = format ["?? none found ?? - %1", _weaponMagsGhost] };
        ...
        // private _loadout = [_path, objNull] call ghost_assignGear_fnc_getLoadoutFromConfig;
