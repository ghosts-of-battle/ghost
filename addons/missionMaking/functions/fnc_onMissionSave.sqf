...
diag_log text "";
diag_log text "------------------------------------------------------";
diag_log text "[GHOST] Saving - Doing mission test:";
diag_log text "------------------------------------------------------";
...
if (!(missionNamespace getVariable [QEGVAR(assignGear,useGhost), false])) exitWith {
    ["Saving: Ghost Gear is disabled"] call BIS_fnc_3DENNotification;
    WARNING("Ghost Gear is disabled");
};
...
if (_missionType == 0) then {
    _problems pushBackUnique ["Need to select mission type", ["GHOST -> Mission Testing Attributes -> Mission Type"]];
};
...
if (_missionLength == 0) then {
    _problems pushBackUnique ["Need to set mission length value", ["GHOST -> Mission Testing Attributes -> Mission Length"]];
};
...
private _checkDeprecatedGear = [];
{
    ...
    if ((side _unit) != civilian) then { // Ignore all civilians
        ...
        if ((primaryWeapon _unit) == "") then {
            if (_typeOf == "ghost_msv_pol") exitWith {};
...
private _fortifies = (all3DENEntities select 3) select {_x isKindOf "ghost_fortify_setupModule"};
{
