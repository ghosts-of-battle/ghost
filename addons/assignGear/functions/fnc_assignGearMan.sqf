 ...
 *
 * Example:
 * [player] call ghost_assignGear_fnc_assignGearMan;
 *
 * Public: Yes
 */
...
if ((!isClass _path) && {(_typeOf select [0,7]) == "ghost_"}) then {
    private _converstionType = 0;
    if (_faction =="ghost_w" && {isClass (missionConfigFile >> "CfgLoadouts" >> "blu_f")}) then {
        _faction ="blu_f";
        _converstionType = 1;
    };
    if (_faction =="ghost_i" && {isClass (missionConfigFile >> "CfgLoadouts" >> "ind_f")}) then {
        _faction ="ind_f";
        _converstionType = 1;
    };
    if (_faction =="ghost_e" && {isClass (missionConfigFile >> "CfgLoadouts" >> "opf_f")}) then {
        _faction ="opf_f";
        _converstionType = 1;
    };
    if (_faction =="ghost_e" && {isClass (missionConfigFile >> "CfgLoadouts" >> "ghost_msv")}) then {
        _faction ="ghost_msv";
        _converstionType = 2;
    };
    if (_converstionType == 0) exitWith { INFO_1("No BWC Factions %1",_typeOf); };
    private _unitRole = toLower _typeOf select [9]; // simple because ghost_x_ is const length
    switch (_converstionType) do {
        ...
        case (2): {
            _loadout = switch (_unitRole) do {
                case"rifleman": {"ghost_msv_rifleman"};
                case"ftl": {"ghost_msv_sr"};
                case"coy";
                case"xo";
                case"sl": {"ghost_msv_plt"};
                case"coy": {"ghost_msv_coy"};
                case"ar": {"ghost_msv_ar"};
                case"aar": {"ghost_msv_ar"};
                case"lat": {"ghost_msv_g"};
                case"cm";
                case"sm": {"ghost_msv_sm"};
                case"matg": {"ghost_msv_matg"};
                case"matag": {"ghost_msv_matag"};
                case"pilot": {"ghost_msv_pilot"};
                case"demo": {"ghost_msv_eng"};
                case"vicc";
                case"vicd": {"ghost_msv_vicd"};
                case"uav": {"soldier_UAV_F"};
                default { WARNING("Untranslated type");  "ghost_msv_rifleman" };
            };
