 ...
 *
 * Example:
 * [_weaponArray, _config, _classname, _doOpticCheck] call ghost_assignGear_fnc_setWeaponAttachement;
 *
 * Public: No
 */
...
if (_typeNumber == OPTIC_TYPE) exitWith {
    if (!GVAR(allowMagnifiedOptics) && {_doOpticCheck} && {[_classname] call FUNC(isOpticMagnified)}) then {
        diag_log text format ["[GHOST-assignGear] allowMagnifiedOptics is false, not adding %1", _classname];
        _classname = "";
    };
...
diag_log text format ["[GHOST-assignGear] - Attachment [%1] has unknown type", _classname];
