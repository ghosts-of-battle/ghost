 ...
 *
 * Example:
 * [cursorTarget] call ghost_assignGear_fnc_assignGearVehicle;
 *
 * Public: Yes
 */
...
if (!isClass _path) then {
    ...
    private _vehConfigFactions = switch (_vehConfigSide) do {
        case (west): { ["blu_f", "ghost_w"] };
        case (east): { ["opf_f", "ghost_e", "ghost_msv"] }; // ghost_msv is depcrecated but kept for BWC for now
        case (independent): { ["ind_f", "ghost_i"] };
        default { ["civ_f"] };
    };
...
if (!isClass _path) exitWith {
    diag_log text format ["[GHOST-assignGear] - No loadout found for %1 (typeOf %2) (kindOf %3) (defaultLoadout: %4)", _theVehicle, typeof _theVehicle, _loadout, _defaultLoadout];
};
