...
GVAR(useGhost) = [missionConfigFile >> "CfgLoadouts" >> "useGhost"] call CFUNC(getBool);

if (GVAR(useGhost)) then {
    ...
    if (isServer) then {
        if (is3DEN) then {
            diag_log text format ["[GHOST-assignGear] Running assign gear for the 3den entities at load"];
    ...
    diag_log text format ["[GHOST-assignGear] Enabled [useFallback: %1, allowMagnifiedOptics: %2, allowChangeableOptics: %3, maxRandomization: %4, setVehicleLoadouts: %5, prefixes: %6]", GVAR(useFallback), GVAR(allowMagnifiedOptics), GVAR(allowChangeableOptics), GVAR(maxRandomization), GVAR(setVehicleLoadouts), GVAR(prefixes)];
} else {
    GVAR(allowChangeableOptics) = false;
    diag_log text format ["[GHOST-assignGear] Disabled"];
};
