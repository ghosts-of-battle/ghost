...
TRACE_3("", missionNamespace getVariable QGVAR(enabled), hasInterface, EGVAR(assignGear,useGhost));
...
if (missionNamespace getVariable [QGVAR(enabled), false]) then {
    ...
    if (isServer) then { // Warn if not enabled
        [{
            ["ghost_adminMsg", ["Warning: Ghost Radios Setting Disabled (This should never happen on a GHOSTMF mission)", "Server"]] call CBA_fnc_globalEvent;
...
if (hasInterface) then {
    ...
    [{
        ...
        INFO_1("[ACRE_MAIN_INIT: %1]",ACRE_MAIN_INIT);
        INFO_1("[acre_player: %1]",acre_player);
        if (!ACRE_MAIN_INIT) then {
            ace_player createDiaryRecord ["diary", ["ACRE ERROR", format ["~~~~~~~~~~~~~~ ACRE Debug ~~~~~~~~~~~~~~<br/>Map: %1<br/>Data: %2<br/>TSID: %3<br/>Main: %4", ACRE_MAP_LOADED, ACRE_DATA_SYNCED, acre_sys_core_ts3id, ACRE_MAIN_INIT]]];
        };
