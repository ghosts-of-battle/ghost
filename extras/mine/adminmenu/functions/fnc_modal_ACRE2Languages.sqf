...
_ctrlLabelLangs ctrlSetPosition [0, 0, (0.5 * _ctrlGrpWidth), GHOST_ADMINMENU_STD_HEIGHT];
...
if (count acre_sys_core_languages == 0) exitWith {
    ...
    GVAR(utilityTabControls) pushBack _ctrlLabelLangs;
    _ctrlLabelLangs ctrlSetPosition [0, 2.1 * GHOST_ADMINMENU_STD_HEIGHT, (0.25 * _ctrlGrpWidth), GHOST_ADMINMENU_STD_HEIGHT];
    _ctrlLabelLangs ctrlCommit 0;
    _ctrlLabelLangs ctrlSetText "No languages configured for mission!";
};

private _langY = 2.1 * GHOST_ADMINMENU_STD_HEIGHT;
...

{
    ...
    _ctrlLabelName ctrlSetPosition [0, _langY + (_forEachIndex * (1.1 * GHOST_ADMINMENU_STD_HEIGHT)), (0.25 * _ctrlGrpWidth), GHOST_ADMINMENU_STD_HEIGHT];
    ...
    _ctrlComboAction ctrlSetPosition [0.5 * _ctrlGrpWidth, _langY + (_forEachIndex * (1.1 * GHOST_ADMINMENU_STD_HEIGHT)), 0.5 * _ctrlGrpWidth, GHOST_ADMINMENU_STD_HEIGHT];
