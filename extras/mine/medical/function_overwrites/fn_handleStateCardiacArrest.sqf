...
if (_timeDiff >= 1) then {
    ...
    private _totalTimeLeft = _unit getVariable ["GHOST_REVIVAL", GHOST_medical_persistantReviveTimer];
    if(GHOST_medical_persistantRevive) then {
    ...
    _unit setVariable ["GHOST_REVIVAL", _totalTimeLeft, true];
};