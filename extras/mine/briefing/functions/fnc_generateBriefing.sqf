...
private _briefingArray = getMissionConfigValue ["GHOST_Briefing","[]"];
...
private _groupCond = _unitGroup getVariable ["GHOST_Briefinglist", []];
if (_groupCond isEqualType "") then { _groupCond = call compile _groupCond; };
private _unitCond = _unit getVariable ["GHOST_Briefinglist", []];
...

{
    ...
    if ((_scriptName) call _fnc_fileExists) then {
        ...
        [_scriptName] spawn {
            params ["_scriptName"];
            uiSleep 5;
            systemChat format["[GHOST-Briefing] Missing file: %1 ", _scriptName];
        };
    };
} forEach _indexesToTrigger;

//Do Admin briefing.
if ([] call ghost_common_fnc_isAdmin) then {
    if (("briefing\admin.sqf") call _fnc_fileExists) then {
        call compile preprocessfilelinenumbers "briefing\admin.sqf";
    } else {
        [] spawn {
            uiSleep 5;
            systemChat "[GHOST-Briefing] Warning admin briefing not found. Expected: MISSION_ROOT\briefing\admin.sqf";
        };
    };
};
